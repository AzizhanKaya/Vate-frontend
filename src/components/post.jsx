...
import { likePost, deletePost } from '@/utils/requests'
import { timestampToDate, formatPost, nthPost, getSubPostHash, getPostHash, base64ToHex, hexToBase64, addLinkedList } from '../utils/encoder';
import { useContext } from 'react';
import { SubPostContext } from '../contexts/sub-post';
import { useAccount } from '../store/auth/hooks';
import { get_time, sign, get_hash } from '@/wasm/wasm'
import { savePostHash, getPostByHash, deletePostByHash } from '@/utils/database';
...
export default function Post({ post }) {
    ...
    if (!subPost) {
        if (setSubPost) setSubPost(post);
    }else if (subPost == post){
        //Already in post
    }
    else {
        setSubPost(prevSubPost => {
        let copy_sub_post = {...prevSubPost};
        let last_sub_post = nthPost(copy_sub_post,-1);
    ...
    function createLike(post) {
        ...
        const like_post = (post != subPost ? formatPost(subPost ? addLinkedList(subPost, post) : post) : formatPost(post));
        
        const like = {
            past_hash: getSubPostHash(like_post),
        ...
        const hash = getPostHash(like);
        like.sign = sign(base64ToHex(account.priv_key), hash);
    ...
    async function handleLike() {
        ...
        if (!account.priv_key) return;
        
        const { like, like_post } = createLike(post);

        if (await isLiked(like_post)) {
            await deleteLike(like_post);
            return;
        }
    
        nthPost(like_post, -1).post = structuredClone(like);
        const res = await likePost(like_post);
        delete nthPost(like_post,-2).post;

        if (res.ok) {
            post.likes += 1;
            setRefresh(prev => !prev);
            saveLike(like_post, like);
        }
    }

    function saveLike(like_post, like) {
        const hash = getSubPostHash(like_post);

        savePostHash(hash, like_post, like);
    }

    async function isLiked(like_post) {
    
        const hash = getSubPostHash(like_post);
        const result = await getPostByHash(hash);
        
        return !!result;
    }

    async function deleteLike(like_post){

        let hash = getSubPostHash(like_post);
        const like = (await getPostByHash(hash)).like;
        const db_res = await deletePostByHash(hash);

        nthPost(like_post, -1).post = like;

        hash = getSubPostHash(like_post);
        const padding = Math.random().toString();
        hash = get_hash(`${hash}:${padding}`);
        ...
        const res = await deletePost(like_post, headers);
    ...
    useEffect(() => {
        async function checkIfLiked() {
            const { like_post } = createLike(post);
            setLiked(await isLiked(like_post));
        }
        if(account?.priv_key){
            checkIfLiked();
        }
    ...
    <div className="px-4 pt-3 gap-3 border border-[#2f3336] flex rounded-xl bg-black z-10">
        ...
        <div className="flex-1 max-w-[%90]">
            <header className="leading-5 flex gap-2 items-center mb-0.5">
                <NavLink to={`/user/${post.account.pub_key}`} className="hover:underline font-bold">
